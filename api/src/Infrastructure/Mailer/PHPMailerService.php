...
class PHPMailerService implements MailerInterface
{
    ...
    public function send(AbstractEmailTemplate $template): void
    {
        try {
            ...
            $this->mailer->send();
            $this->logger->info(sprintf("E-mail '%s' enviado para %s", $template->getTemplatePath(), $template->getToEmail()));
        } catch (PHPMailerException $e) {
            $this->logger->error(sprintf("Falha ao enviar e-mail '%s' para %s: %s", $template->getTemplatePath(), $template->getToEmail(), $e->getMessage()));

            throw new EmailSendingFailedException('Falha ao enviar e-mail: ' . $e->getMessage(), (int)$e->getCode(), $e);
        } catch (Throwable $e) {
            $this->logger->error(sprintf("Ocorreu um erro inesperado ao enviar e-mail '%s' para %s: %s", $template->getTemplatePath(), $template->getToEmail(), $e->getMessage()));

            throw new EmailSendingFailedException('Ocorreu um erro inesperado: ' . $e->getMessage(), (int)$e->getCode(), $e);
        }
    }
    ...
    private function configureMailer(): void
    {
        try {
            ...
            $this->logger->error('Falha ao configurar PHPMailer: ' . $phpMailerException->getMessage());

            throw new EmailSendingFailedException('Erro de configuração do Mailer: ' . $phpMailerException->getMessage(), (int)$phpMailerException->getCode(), $phpMailerException);
        }
    }
    ...
    private function renderHtmlTemplate(AbstractEmailTemplate $abstractEmailTemplate): string
    {
        ...
        if (!\file_exists($contentTemplatePath)) {
            $this->logger->error('Template de conteúdo de e-mail não encontrado: ' . $contentTemplatePath);

            throw new EmailSendingFailedException('Template de conteúdo de e-mail não encontrado: ' . $contentTemplatePath);
        }

        // Ensure the layout template file exists
        if (!\file_exists($layoutTemplatePath)) {
            $this->logger->error('Template de layout de e-mail não encontrado: ' . $layoutTemplatePath);

            throw new EmailSendingFailedException('Template de layout de e-mail não encontrado.');
        }
