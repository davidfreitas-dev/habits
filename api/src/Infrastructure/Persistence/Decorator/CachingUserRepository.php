...
class CachingUserRepository implements UserRepositoryInterface
{
    ...
    public function findById(int $id): ?User
    {
        ...
        if ($cachedUser) {
            $this->logger->info('Cache de usuário encontrado para o ID: ' . $id);

            return User::fromArray(\json_decode((string) $cachedUser, true));
        }

        $this->logger->info('Cache de usuário não encontrado para o ID: ' . $id);
    ...
    public function findByEmail(string $email): ?User
    {
        ...
        $cachedUser = $this->cache->get($cacheKey);
        if ($cachedUser) {
            $this->logger->info('Cache de usuário encontrado para o e-mail: ' . $email);

            return User::fromArray(\json_decode((string) $cachedUser, true));
        }

        $this->logger->info('Cache de usuário não encontrado para o e-mail: ' . $email);
    ...
    public function update(User $user): User
    {
        ...
        $this->logger->info('Usuário atualizado proativamente no cache para o ID: ' . $updatedUser->getId());
    ...
    public function markUserAsVerified(int $userId): void
    {
        ...
        $this->logger->info('Usuário marcado como verificado no BD, cache invalidado para o ID: ' . $userId);
    }
    ...
    private function invalidateCache(User $user): void
    {
        ...
        $this->logger->info('Cache invalidado para o ID de usuário: ' . $user->getId());
    }
}
