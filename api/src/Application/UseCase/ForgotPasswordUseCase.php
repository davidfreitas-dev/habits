...
class ForgotPasswordUseCase
{
    ...
    public function execute(ForgotPasswordRequestDTO $request): void
    {
        ...
        if (!$user instanceof \App\Domain\Entity\User) {
            $this->logger->info('Tentativa de recuperação de senha para e-mail desconhecido', ['email' => $request->getEmail()]);
        ...
        if (!$person) {
            $this->logger->error(sprintf('Usuário com e-mail %s não possui dados de pessoa associados.', $user->getEmail()), ['user_id' => $user->getId()]);
        ...
        $this->logger->info('Código de redefinição de senha gerado e salvo para o ID de usuário: ' . $user->getId());
        ...
        try {
            ...
            $this->logger->info('E-mail de redefinição de senha enviado', ['email' => $user->getEmail()]);
        } catch (EmailSendingFailedException $emailSendingFailedException) { // Changed to EmailSendingFailedException
            $this->logger->error(sprintf('Falha ao enviar e-mail de redefinição de senha para %s: %s', $user->getEmail(), $emailSendingFailedException->getMessage()));
