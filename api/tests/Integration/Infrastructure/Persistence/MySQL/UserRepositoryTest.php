...
class UserRepositoryTest extends DatabaseTestCase
{
    ...
    private function createTestUser(int $roleId = 1): User
    {
        ...
        if (!$role instanceof Role) {
            throw new NotFoundException(sprintf("Perfil com ID %d não encontrado. Certifique-se de que foi semeado.", $roleId));
        }
    ...
    {
        $createdUser = $this->createTestUser();
        $this->assertFalse($createdUser->isVerified(), 'O usuário não é verificado por padrão'); // User is not verified by default
