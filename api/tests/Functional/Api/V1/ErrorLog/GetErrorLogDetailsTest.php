...
class GetErrorLogDetailsTest extends FunctionalTestCase
{
    ...
    private function createAdminUser(): void
    {
        ...
        if (!$adminRole) {
            $this->fail('Perfil "admin" não encontrado no banco de dados.');
        }
    ...
    private function createRegularUser(): void
    {
        ...
        if (!$userRole) {
            $this->fail('Perfil "user" não encontrado no banco de dados.');
        }
    ...
    public function testGetErrorLogDetailsAsAdmin(): void
    {
        ...
        $this->assertEquals('Critical error 1', $body['data']['message']);
    }

    public function testGetErrorLogDetailsForNotFound(): void
    {
        $response = $this->sendRequest(
            'GET',
            '/api/v1/error-logs/999',
            headers: ['Authorization' => 'Bearer ' . $this->adminToken]
        );
    ...
    public function testGetErrorLogDetailsAsUserFails(): void
    {
        $response = $this->sendRequest(
            'GET',
            '/api/v1/error-logs/1',
