...
class RegisterTest extends FunctionalTestCase
{
    ...
    public function testShouldSuccessfullyCompleteUserRegistrationFlow(): void
    {
        ...
        $this->assertFalse($decodedInitialToken->is_verified, 'O token inicial deve ser restrito');

        // Step 3: Retrieve user and verification token
        /** @var User|null $user */
        $user = $this->userRepository->findByEmail($registerPayload['email']);
        $this->assertNotNull($user, 'O usuário deve existir após o registro');
        $this->assertFalse($user->isVerified(), 'O usuário não deve ser verificado inicialmente');

        // Get verification token directly from database
        $verificationToken = $this->getLatestVerificationTokenForUser($user->getId());
        $this->assertNotEmpty($verificationToken, 'O token de verificação deve existir');
        ...
        $this->assertTrue($decodedFinalToken->is_verified, 'O token final não deve ser restrito');

        // Step 6: Verify user status in database
        $updatedUser = $this->userRepository->findById($user->getId());
        $this->assertNotNull($updatedUser, 'O usuário atualizado deve existir');
        $this->assertTrue($updatedUser->isVerified(), 'O usuário deve ser verificado após a confirmação do e-mail');
    }
