...
class UpdateHabitTest extends FunctionalTestCase
{
    ...
    public function testUpdateReturnsOk(): void
    {
        ...
        $updatedTitle = $this->faker->sentence(4);
        $updatedWeekDays = [0, 6]; // Sunday, Saturday
        ...
        // Act
        $response = $this->sendRequest('PUT', "/api/v1/habits/{$habitId}", $payload, [
            'Authorization' => 'Bearer ' . $this->accessToken,
        ]);
        ...
        $this->assertEquals('Hábito atualizado com sucesso.', $body['message']);
        $this->assertArrayHasKey('data', $body);
        $this->assertEquals($habitId, $body['data']['id']);
        $this->assertEquals($updatedTitle, $body['data']['title']);
        $this->assertEquals($updatedWeekDays, $body['data']['week_days']);
        ...
        $this->assertEquals($updatedTitle, $dbHabit['title']);

        $dbWeekDays = $this->getWeekDaysFromDatabase($habitId);
        $this->assertEquals($updatedWeekDays, $dbWeekDays);
    }
}