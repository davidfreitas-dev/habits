...
final class CachingUserRepositoryTest extends TestCase
{
    ...
    public function testFindByIdCacheHit(): void
    {
        ...
        $logger->expects($this->once())
            ->method('info')
            ->with('Cache de usuário encontrado para o ID: ' . $user->getId());
    ...
    public function testFindByIdCacheMiss(): void
    {
        ...
        $logger->expects($this->once())
            ->method('info')
            ->with('Cache de usuário não encontrado para o ID: ' . $user->getId());
    ...
    public function testFindByIdNotFound(): void
    {
        ...
        $logger->expects($this->once())
            ->method('info')
            ->with('Cache de usuário não encontrado para o ID: ' . $userId);
    ...
    public function testFindByEmailCacheHit(): void
    {
        ...
        $logger->expects($this->once())
            ->method('info')
            ->with('Cache de usuário encontrado para o e-mail: ' . $user->getEmail());
    ...
    public function testFindByEmailCacheMiss(): void
    {
        ...
        $logger->expects($this->once())
            ->method('info')
            ->with('Cache de usuário não encontrado para o e-mail: ' . $user->getEmail());
    ...
    public function testFindByEmailNotFound(): void
    {
        ...
        $logger->expects($this->once())
            ->method('info')
            ->with('Cache de usuário não encontrado para o e-mail: ' . $userEmail);
    ...
    public function testUpdateInvalidatesOldCacheAndRecachesNewUser(): void
    {
        ...
        $logger->expects($this->exactly(3))
            ...
            ->willReturnOnConsecutiveCalls(
                $this->equalTo('Cache de usuário não encontrado para o ID: ' . $originalUser->getId()),
                $this->equalTo('Cache invalidado para o ID de usuário: ' . $originalUser->getId()),
    ...
    public function testDeleteInvalidatesCache(): void
    {
        ...
        $logger->expects($this->exactly(2))
            ->method('info')
            ->willReturnOnConsecutiveCalls(
                $this->equalTo('Cache de usuário não encontrado para o ID: ' . $userToDelete->getId()),
                $this->equalTo('Cache invalidado para o ID de usuário: ' . $userToDelete->getId())
            );
    ...
    public function testUpdatePasswordInvalidatesCache(): void
    {
        ...
        $logger->expects($this->exactly(2))
            ->method('info')
            ->willReturnOnConsecutiveCalls(
                $this->equalTo('Cache de usuário não encontrado para o ID: ' . $user->getId()),
                $this->equalTo('Cache invalidado para o ID de usuário: ' . $user->getId())
            );
        ...
        $logger->expects($this->exactly(3))
            ...
            ->willReturnCallback(function ($message) use ($user) {
                ...
                $validMessages = [
                    ...
                    'Cache invalidado para o ID de usuário: ' . $user->getId(),
                    'Usuário marcado como verificado no BD, cache invalidado para o ID: ' . $user->getId()
                ];
