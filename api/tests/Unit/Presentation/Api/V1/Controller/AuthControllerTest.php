...
class AuthControllerTest extends TestCase
{
    ...
    public function testRegisterEmailSendingFailedException(): void
    {
        ...
        $this->jsonResponseFactory->expects($this->once())
            ->method('success')
            ->willThrowException(new EmailSendingFailedException('Falha ao enviar e-mail de boas-vindas.'));
    ...
    public function testRefreshWithMissingToken(): void
    {
        ...
        $mockedResponse->getBody()->write(json_encode([
            'status' => 'fail',
            'data' => null,
            'message' => 'Token inválido' // Expected message from AuthController's catch block
        ]));
        $this->jsonResponseFactory->expects($this->once())
            ->method('fail')
            ->with(null, 'Token inválido', 401) // Expect the message from the exception
            ->willReturn($mockedResponse);
